...
exports.getOneCategory = async (req, res) => {
    try {
        const userId = await getUserIdByUserName(req.params.userName);
        const category = await budgetCategoryService.getCategoryByName(req.params.categoryName, userId);
...
exports.createCategory = async (req, res) => {
    ...
    try {
        const userId = await getUserIdByUserName(req.params.userName);
        const newCategory = await budgetCategoryService.createCategory(categoryName, userId);
...
exports.updateCategory = async (req, res) => {
    ...
    try {
        const userId = await getUserIdByUserName(req.params.userName);
        const updated = await budgetCategoryService.updateCategory(categoryId, categoryName, userId);
        res.json(updated);
    } catch (error) {
        if (error.message === 'Category not found or user not authorized') {
